...
{strip}
    ...
    <div class="relatedContainer">
        ...
        {if $MODULE eq 'Contacts' && $RELATED_MODULE_NAME neq 'TradesCommission'}
            {include file="partials/RelatedListHeader.tpl"|vtemplate_path:$RELATED_MODULE_NAME}
        {/if}
        {if $MODULE eq 'Contacts' && $RELATED_MODULE_NAME eq 'TradesCommission'}
            <style>
            .textOverflowEllipsis { overflow: visible !important;}
            </style>
        {/if}
            ...
            <div class="bottomscroll-div">
                <table id="listview-table" class="table listview-table">
                    <thead>
                        <tr class="listViewHeaders">
                            ...
                            {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                                {if $HEADER_FIELD->getName() eq 'custom_data'} {continue} {/if}
                        ...
                        <tr class="searchRow {if $MODULE eq 'Contacts' && $RELATED_MODULE_NAME eq 'TradesCommission'} hide {/if}">
                    ...
                    {foreach item=RELATED_RECORD from=$RELATED_RECORDS}
                        <tr {if  $RELATED_MODULE_NAME neq 'Payments'} class="listViewEntries" {/if}  data-id='{$RELATED_RECORD->getId()}' 
                                                                      {if $RELATED_MODULE_NAME eq 'Calendar'}
                                                                          ...
                                                                          {/if}
                                                                      {else if $RELATED_MODULE_NAME neq 'TradesCommission'}
                                                                          data-recordUrl='{$RELATED_RECORD->getDetailViewUrl()}'
                                                                      {/if} >
                            <td class="related-list-actions">
                                {*add by divyesh 06-03-2019
                                remove delete link and edit link*}
                                {assign var=HIDE_EDIT_DELETE_LINK value=true}
                                {if $RELATED_MODULE_NAME eq 'LiveAccount' || $RELATED_MODULE_NAME eq 'DemoAccount' || $RELATED_MODULE_NAME eq 'Documents' || $RELATED_MODULE_NAME eq 'Payments' || $RELATED_MODULE_NAME eq 'Ewallet' || $RELATED_MODULE_NAME eq 'LeverageHistory' || $RELATED_MODULE_NAME eq 'TradesCommission' || $RELATED_MODULE_NAME eq 'HelpDesk'}
                                    {assign var=HIDE_EDIT_DELETE_LINK value=$RELATED_RECORD->checkRecordStatus($RELATED_RECORD->getId())}
                                {/if}
                                
                                {if $RELATED_MODULE_NAME eq 'Payments'}
                                    {assign var=PAYMENT_CUSTOM_DATA value=$RELATED_RECORD->get('custom_data')}
                                    {assign var=PAYMENT_RECIEPT_DOC_ID value=$RELATED_RECORD->getReceiptDocId($PAYMENT_CUSTOM_DATA)}
                                {/if}

                                <span class="actionImages">&nbsp;&nbsp;&nbsp;
                                    {if $IS_EDITABLE && $RELATED_RECORD->isEditable() && $HIDE_EDIT_DELETE_LINK }
                                        ...
                                        {if $RELATED_MODULE_NAME eq 'PriceBooks'}
                                            ...
                                            {else if $MODULE eq 'Products' && $RELATED_MODULE_NAME eq 'Products' && $TAB_LABEL === 'Product Bundles' && $RELATED_LIST_LINKS && $PARENT_RECORD->isBundle()}
                                               ...
                                               onclick ="Products_Detail_Js.triggerEditQuantity('index.php?module=Products&view=SubProductQuantityUpdate&record={$PARENT_RECORD->getId()}&relid={$RELATED_RECORD->getId()}&currentQty={$quantity}');if (event.stopPropagation){ldelim}
                                                           event.stopPropagation();{rdelim} else{ldelim}
                                                                       event.cancelBubble = true;{rdelim}"
                                                {else}
                                                    {if  $RELATED_MODULE_NAME eq 'Payments'}
                                                        <a name="relationEdit" href="{$RELATED_RECORD->getEditViewUrl()}" target="_blank"
                                                        {else}
                                                ...
                                                {if $RELATED_MODULE_NAME eq 'Payments' && !empty($PAYMENT_RECIEPT_DOC_ID)}
                                                    <span class="value" data-field-type="documentsFileUpload">
                                                        <a name="viewfile" href="javascript:void(0)" data-filelocationtype="I" data-filename="" onclick="Vtiger_Header_Js.previewFile(event,{$PAYMENT_RECIEPT_DOC_ID})">
                                                        <i title="View File" class="fa fa-picture-o alignMiddle"></i>&nbsp;</a>
                                                    </span>
                                                {/if}
                                                
                                                {*{if $IS_DELETABLE && $HIDE_EDIT_DELETE_LINK}

    ...
    */
	...
	registerEventForPicklistDependencySetup : function(container){
		var picklistDependcyElemnt = jQuery('[name="picklistDependency"]',container);
                ...
                {/*add for payment picklist dependency issue resolve in edit mode*/
                    var recordId = jQuery('[name="record"]',container).val();
                    if(picklistDependcyElemnt.length <= 0 || recordId !== ''){return;}
                }
                else 
                {
                    if(picklistDependcyElemnt.length <= 0) {return;}
                }

    ...
    <div name='editContent'>
        ...
        {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=blockIterator}
            {if $BLOCK_FIELDS|@count gt 0}
                <div class='fieldBlockContainer' data-block="{$BLOCK_LABEL}" {if $BLOCK_LABEL eq  'LBL_IBCOMMISSION_COMBINATION_ITEMS'} style="padding-top: 17px;"{/if}>
                    {if $BLOCK_LABEL eq 'LBL_IBCOMMISSION_COMBINATION_ITEMS'}

                        <div class="col-lg-12" style="padding:0px">                            
                            <span class='fieldBlockHeader' style="font-size: 18px;">{vtranslate($BLOCK_LABEL, $MODULE)}</span>
                            <div  style="float: right;">
                                <span id="addIBCommission" class="addIBCommission  btn btn-default" title="Create" style="float: right;margin-left: 10px;">{vtranslate('LBL_ADD_ITEMS', $MODULE)}</span>
{*                                <span id="updateIBCommission" class="updateIBCommission updateReferenceRecord cursorPointer clearfix btn btn-default" title="Update" style="float: right;">{vtranslate('LBL_UPDATE_ITEMS', $MODULE)}</span>*}
                            </div>
                            <hr>
                        </div>
                        <input type="hidden" id="IBCommissionCobinationJson" value="">
                        <table class="table table-borderless  IBCommissionItemDetails" id ="IBCommissionItemDetails">
                            <tbody>
                                <tr>
                                    <td class="" >
                                        {vtranslate('LBL_METATRADER_TYPE', $MODULE_NAME)}&nbsp;&nbsp;
                                    </td>
                                    <td class="" >
                                        {vtranslate('LBL_IBCOMMISSION_LEVEL', $MODULE_NAME)}&nbsp;&nbsp;
                                    </td>
                                    <td class="" >
                                        {vtranslate('LBL_SECURITY', $MODULE_NAME)}&nbsp;&nbsp;
                                    </td>
                                    <td class="" >
                                        {vtranslate('LBL_SYMBOL', $MODULE_NAME)}&nbsp;&nbsp;
                                    </td>
                                    <td class="" >
                                        {vtranslate('LBL_LABEL_ACCOUNT_TYPE', $MODULE_NAME)}&nbsp;&nbsp;
                                    </td>
                                    <td class="" >
                                        {vtranslate('LBL_CURRENCY_CODE', $MODULE_NAME)}&nbsp;&nbsp;
                                    </td>
                                    <td class="" >
                                        {vtranslate('LBL_IB_COMMISSION_TYPE', $MODULE_NAME)}&nbsp;&nbsp;
                                    </td>
                                    <td class="" >
                                        {vtranslate('LBL_IB_COMMISSION_VALUE', $MODULE_NAME)}&nbsp;&nbsp;
                                    </td>
                                </tr>
                                {if $RECORD_ID}
                                    {include file="partials/EditViewIBCommissionCombinationItems.tpl"|@vtemplate_path:$MODULE}
                                {/if}
                            </tbody>
                        </table>
                    {else}
                            ...
                            <tr>
                                ...
                                {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                                    ...
                                    {if $FIELD_MODEL->isEditable() eq true}
                                        ...
                                        {/if}
                                        <td id="fieldLabel_{$FIELD_NAME}" class="fieldLabel alignMiddle {$FIELD_NAME}">

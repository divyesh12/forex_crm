 ...
 ************************************************************************************}
{*
Change By Divyesh Chothani
Date:- 24-12-2019
Commnet:- Add Field Id of td row
*}
{strip}
							...
							<div id="upload" data-filelocationtype="I">
								...
								<table class="massEditTable table no-border">
									<tr>
										...
										<td id="fieldLabel_{$FIELD_MODEL->get('name')}" class="fieldLabel col-lg-2">
										...
										<td id="fieldValue_{$FIELD_MODEL->get('name')}" class="fieldValue col-lg-4" colspan="3">
									...
									<tr>
										...
										<td id="fieldValue_{$FIELD_MODEL->get('name')}" class="fieldValue col-lg-4">
											{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE)}
										</td>
									...
									<tr>
										...
										{foreach key=FIELD_NAME item=FIELD_MODEL from=$FIELD_MODELS} 
											{if !in_array($FIELD_NAME,$HARDCODED_FIELDS) && $FIELD_MODEL->isQuickCreateEnabled()}
												...
												{if $FIELD_MODEL->get('uitype') neq '83'}
													<td id="fieldValue_{$FIELD_MODEL->get('name')}" class="fieldValue col-lg-4" {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
														{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE)}
													</td>
												{/if}
											{/if}
										...
										{if $FIELD_MODELS['notecontent']}
											<td id="fieldLabel_{$FIELD_MODEL->get('name')}" class="fieldLabel col-lg-2" colspan="1">
												<label class="muted pull-right">
													{vtranslate($FIELD_MODEL->get('label'), $MODULE)}&nbsp;
													{if $FIELD_MODEL->isMandatory() eq true}
														<span class="redColor">*</span>

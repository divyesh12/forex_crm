...
Vtiger_Edit_Js("Documents_Edit_Js", {
    ...
    /**
     ...
     @Date:-13-09-2019
     @Comment:-Mandatory documents Reject Reason while Document status is Rejected
     */
    ...
    /**
     @Created_by:-Divyesh Chothani
     @Date:-13-09-2019
     @Comment:-Documents type ristriction except pdf and image format in stox11
     */
    registorFieldValidation: function (container) {
        jQuery(document).on('change', 'input[name="filename"]', function (container) {
            var fileName = Documents_Edit_Js.file.name;
            var fileSize = Documents_Edit_Js.file.size;
            var type = Documents_Edit_Js.file.type;
            var filesplit = type.split('/');
            var max_upload_limit = 3145728;
            var fileExtensionList = ['jpeg', 'jpg', 'png', 'pdf'];

            if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtensionList) == -1) {
                var fileExtensionList = fileExtensionList.join(', ');
                jQuery('input[name="filename"]').val('');
                jQuery('div.uploadedFileSize').html('');
                ...
                jQuery('input[name="filename"]').html('<span style="color:black;">' + fileName + '</span>');
            }
        }
        );
    },
    ...
    registerBasicEvents: function (container) {
        ...
        this.registerDocumentStatusChangeEvent(container);
        this.registorFieldValidation(container);
        //end
    },
});

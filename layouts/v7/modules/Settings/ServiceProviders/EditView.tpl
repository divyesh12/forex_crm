...
{strip}
    <div class="modal-dialog modal-xs">
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="smsConfig" method="POST" enctype="multipart/form-data">
                <div class="modal-body configContent">
                    ...
                    {foreach item=FIELD_MODEL from=$EDITABLE_FIELDS}
                        <div class="col-lg-12">
                            ...
                            <div class="form-group">
                                <div class = "col-lg-4">
                                    <label for="{$FIELD_NAME}">{vtranslate($FIELD_NAME, $QUALIFIED_MODULE_NAME)}<span class="redColor">*</span></label>
                                </div>
                                <div class = "col-lg-6">
                                    {assign var=FIELD_TYPE value=$FIELD_MODEL->getFieldDataType()}
                                    {assign var=FIELD_VALUE value=$RECORD_MODEL->get($FIELD_NAME)}
                                    {if $FIELD_TYPE == 'picklist'}
                                        <select {if $FIELD_VALUE && $FIELD_NAME eq 'providertype'} disabled="disabled" {/if} class="select2 providerType form-control" id="{$FIELD_NAME}" name="{$FIELD_NAME}" placeholder="{vtranslate('LBL_SELECT_OPTION', $QUALIFIED_MODULE_NAME)}" required>
                                            <option></option>
                                    ...
                                    {else}
                                        ...
                                        {if $FIELD_NAME eq 'title' and $FIELD_VALUE neq ''}
                                            {assign var=readonly value="readonly"}
                                        {/if}
                                        <input type="text" name="{$FIELD_NAME}" class="form-control" id="{$FIELD_NAME}" {if $FIELD_NAME == 'username'} {/if} value="{$FIELD_VALUE}" required {$readonly}/>
                                    {/if}
                                </div>
                            </div>
                            ...
                            {foreach key=PROVIDER_NAME item=PROVIDER_MODEL from=$PROVIDERS_FIELD_MODELS}
                                {if $PROVIDER_NAME eq $RECORD_MODEL->get('providertype')}
                                    <div id="{$PROVIDER_NAME}_container" class="providerFields">
                                        {assign var=TEMPLATE_NAME value=Settings_ServiceProviders_ProviderField_Model::getEditFieldTemplateName($PROVIDER_NAME)}
                    ...
                    <div>
                        <span id='phoneFormatWarning'> 
                            <!-- <i data-trigger="hover" data-toggle ="popover" data-placement="right" id="phoneFormatWarningPop" class="glyphicon glyphicon-info-sign" style="padding-right : 5px; padding-left : 5px" data-original-title="{vtranslate('LBL_WARNING',$QUALIFIED_MODULE_NAME)}" data-trigger="hover" data-content="{vtranslate('LBL_PHONEFORMAT_WARNING_CONTENT',$QUALIFIED_MODULE_NAME)}"></i> -->
                            {vtranslate('LBL_PHONE_FORMAT_WARNING', $QUALIFIED_MODULE_NAME)}
                        </span>
                    </div>
                </div>

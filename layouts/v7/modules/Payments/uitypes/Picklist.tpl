...
{assign var=PICKLIST_COLORS value=$FIELD_INFO['picklistColors']}
      ...
      {if  ($FIELD_MODEL->getFieldName() eq 'payment_operation' OR $FIELD_MODEL->getFieldName() eq 'payment_type' OR $FIELD_MODEL->getFieldName() eq 'payment_from' OR $FIELD_MODEL->getFieldName() eq 'payment_to' OR $FIELD_MODEL->getFieldName() eq 'payment_currency') && $RECORD_ID}
            <select id="field_{$FIELD_MODEL->getFieldName()}" data-fieldname="{$FIELD_MODEL->getFieldName()}" data-fieldtype="picklist" class="inputElement select2 {if $OCCUPY_COMPLETE_WIDTH} row {/if}" type="picklist" name="{$FIELD_MODEL->getFieldName()}" {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'
	{if $FIELD_INFO["mandatory"] eq true} data-rule-required="true" {/if}
	{if count($FIELD_INFO['validator'])}
		data-specific-rules='{ZEND_JSON::encode($FIELD_INFO["validator"])}'
	{/if}
	>
                  {assign var=PICKLIST_VALUE value=trim(decode_html($FIELD_MODEL->get('fieldvalue')))}
                  <option value="{$PICKLIST_VALUE}" selected="selected">{$PICKLIST_VALUE}</option>
            </select>
      {else}
            <select id="field_{$FIELD_MODEL->getFieldName()}" data-fieldname="{$FIELD_MODEL->getFieldName()}" data-fieldtype="picklist" class="inputElement select2 {if $OCCUPY_COMPLETE_WIDTH} row {/if}" type="picklist" name="{$FIELD_MODEL->getFieldName()}" {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'
                    {if $FIELD_INFO["mandatory"] eq true} data-rule-required="true" {/if}
...
{/if}
      {/if}    
{if $FIELD_MODEL->getFieldName() eq 'payment_type'}
<style>
.payment_type_tooltip {
  position: relative;
  display: inline-block;
  z-index:99999;
}

.payment_type_tooltip .payment_type_tooltiptext {
  visibility: hidden;
  width: 168px;
...
.payment_type_tooltip:hover .payment_type_tooltiptext {
  visibility: visible;
}
</style>
<div class="payment_type_tooltip">&nbsp;&nbsp;<i class="fa fa-info-circle"></i>
  <span class="payment_type_tooltiptext">{vtranslate('LBL_PAYMENT_TYPE_OPTIONS_LABEL',$MODULE)}</span>
</div>
{/if}
{/strip}

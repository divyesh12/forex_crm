        ...
        <div class="relatedHeader">
            <div class="btn-toolbar row-fluid">
                <div class="col-lg-6 col-md-6 col-sm-6">
                    ...
                    {/foreach}
                    &nbsp;
                    {assign var=CAMPAIGN_ACTIVITY_DATA value=CampaignActivity_Module_Model::getCampaignActivityList($PARENT_RECORD->getId())}
                    {if !empty($CAMPAIGN_ACTIVITY_DATA)}
                    <select class="inputElement select2" name="campaign_activity_list_of_email" id="campaign_activity_list_of_email">
                        <option value="" >Select Campaign Activity</option>
                        {foreach item=ACTIVITY_NAME key=ACTIVITY_ID from=$CAMPAIGN_ACTIVITY_DATA}
                            <option value="{$ACTIVITY_ID}" {if $smarty.request.campaign_activity_id eq $ACTIVITY_ID OR $smarty.session.campaign_activity_id eq $ACTIVITY_ID}selected="selected"{/if}>{$ACTIVITY_NAME}</option>
                        {/foreach}
                    </select>
                    {/if}
                </div>
                    ...
                    {foreach item=RELATED_RECORD from=$RELATED_RECORDS}
                        ...
                        <tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' data-emailflag='{$EMAIL_FLAG}' name="emailsRelatedRecord">
                            ...
                            {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                                ...
                                {assign var=RELATED_RECORD_ID value=$RELATED_RECORD->getId()}
                                <td class="{$WIDTHTYPE}" data-val="{$RELATED_RECORD_ID}">
                                    {if $HEADER_FIELD->isNameField() eq true or $HEADER_FIELD->get('uitype') eq '4'}
                                        <a>{$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}</a>
                                    {*{elseif $RELATED_HEADERNAME eq 'access_count'}
                                        {$RELATED_RECORD->getAccessCountValue($RELATED_RECORD_ID)}
                                    {elseif $RELATED_HEADERNAME eq 'click_count'}
                                        {$RELATED_RECORD->getClickCountValue($RELATED_RECORD_ID)}*}

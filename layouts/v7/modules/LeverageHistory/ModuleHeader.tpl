    ...
    <div class="col-sm-12 col-xs-12 module-action-bar clearfix coloredBorderTop">
        <div class="module-action-content clearfix {$MODULE}-module-action-content">
            <div class="col-lg-7 col-md-7 module-breadcrumb module-breadcrumb-{$smarty.request.view} transitionsAllHalfSecond">
                ...
                {*Modified by Divyesh Chothani
                Date:- 31-08-2018
                Comment:- Display Related Module Record Name*}
                {if $RECORD and $smarty.request.view eq 'Edit'}
                    {foreach item=NAME_FIELD from=$MODULE_MODEL->getNameFields()}
                        {assign var=FIELD_MODEL value=$MODULE_MODEL->getField($NAME_FIELD)}
                        {if  $FIELD_MODEL->get('uitype') eq 10}
                            {assign var=LABEL value=$FIELD_MODEL->getEditViewDisplayValue($RECORD->get($NAME_FIELD))}
                        {else}
                            {assign var=LABEL value=$RECORD->get('label')}
                        {/if}
                    {/foreach}
                    <p class="current-filter-name filter-name pull-left "><span class="fa fa-angle-right pull-left" aria-hidden="true"></span><a title="{$LABEL}">&nbsp;&nbsp;{vtranslate('LBL_EDITING', $MODULE)} : {$LABEL} &nbsp;&nbsp;</a></p>
                    ...
                    <p class="current-filter-name filter-name pull-left "><span class="fa fa-angle-right pull-left" aria-hidden="true"></span><a>&nbsp;&nbsp;{vtranslate('LBL_ADDING_NEW', $MODULE)}&nbsp;&nbsp;</a></p>
                        ...
                        {if $smarty.request.view eq 'Detail'}
                            {foreach item=NAME_FIELD from=$MODULE_MODEL->getNameFields()}
                                ...
                                {/if}
                            {/foreach}
                    <p class="current-filter-name filter-name pull-left"><span class="fa fa-angle-right pull-left" aria-hidden="true"></span><a title="{$LABEL}">&nbsp;&nbsp;{$LABEL} &nbsp;&nbsp;</a></p>
                        {/if}
                        {*end*}
            </div>

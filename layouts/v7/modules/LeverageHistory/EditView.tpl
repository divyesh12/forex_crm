                                                ...
                                                {if $RECORD_ID neq ''}
                                                      {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=blockIterator}
                                                            {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                                                                  {if $FIELD_NAME eq 'liveaccountid' and $FIELD_MODEL->get('uitype') eq 10}
                                                                        {assign var=LABEL value=$FIELD_MODEL->getEditViewDisplayValue($RECORD_STRUCTURE_MODEL->getRecordName())}
                                                                        <h4 class="editHeader" style="margin-top:5px;" title="{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} {$RECORD_STRUCTURE_MODEL->getRecordName()}">{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} - {$LABEL}</h4>
                                                                  {/if}
                                                            {/foreach}
                                                      {/foreach}

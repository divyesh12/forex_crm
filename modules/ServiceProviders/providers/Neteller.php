...
include_once 'lib/NetellerAPI.php';

class ServiceProviders_Neteller_Provider extends ServiceProviders_AbstractPaymentGatways_Model {
    ...
    private static $REQUIRED_PARAMETERS = array(
        ...
        array('name' => 'api_key', 'label' => "PRIVATE Key's Base64 Encode", 'type' => 'textarea', 'mandatory' => true),
    ...
    public function getName() {
        return 'Neteller'; // don't take name with any space or special charctor
    }
        ...
        if (!empty($request)) {
            ...
            if ($request['payment_operation'] == 'Deposit') {
                ...
                if ($provider->parameters['test_mode'] == 'Yes') {
                    $base_url = $provider->parameters['test_url'];
                }
                $amount_value = number_format($request['amount'], $provider->parameters['allowed_decimal'], '.', '');
                ...
                $postfields = array(
                    ...
                    'neteller' => array(
                        'consumerId' => $request['email'],
                        'consumerIdLocked' => true,
                        'detail1Description' => $provider->parameters['description'],
                        'detail1Text' => 'Deposit'),
                    'paymentType' => 'NETELLER',
                    'amount' => $amount_value,
                    'currencyCode' => $request['payment_currency'],
                    'customerIp' => $_SERVER['REMOTE_HOST'],
...
//                    ),
                    'returnLinks' => array(
                        array('rel' => 'on_completed', 'href' => $returnUrl),
                        array('rel' => 'on_failed', 'href' => $cancelUrl),
                        array('rel' => 'default', 'href' => $cancelUrl),
                    )
                );
                ...
                $headers = array(
                    ...
                    "Authorization: Basic " . $provider->parameters['api_key'],
                    "Simulator: EXTERNAL");
                ...
                if (empty($response->error)) {
                    $redirectUrl = $response->links[0]->href . '&orid=' . $order_id;
                    $request['redirectUrl'] = $redirectUrl;
                    $request['neteller_response'] = $response;

...
class ServiceProviders_Skrill_Provider extends ServiceProviders_AbstractPaymentGatways_Model {

    protected $module = 'Payments';
    protected $translate_module = 'CustomerPortal_Client'; // Common label file
    private static $REQUIRED_PARAMETERS = array(
        array('name' => 'business_email', 'label' => 'Business Email', 'type' => 'text', 'mandatory' => true),
    );
    ...
    public function getName() {
        return 'Skrill'; // don't take name with any space or special charctor
    }
    ...
    public function paymentProcess($request, $portal_language) {
        ...
        if (!empty($request)) {
            ...
            if ($request['payment_operation'] == 'Deposit') {
                ...
                $parameters = array(
                    'pay_to_email' => $provider->parameters['business_email'],
                    'pay_from_email' => $request['email']
                );
                $gateway->setEmail($provider->parameters['business_email']);
                $response = $gateway->purchase(['cancel_url' => $cancelUrl,
                            'return_url' => $returnUrl,
                            'amount' => $amount_value, 'language' => 'EN',
                            'currency' => $request['payment_currency'],
                            'transaction_id' => $order_id,
                            'details' => $parameters])->send();
                $errors_getway = $response->getMessage();
                if (empty($errors_getway)) {

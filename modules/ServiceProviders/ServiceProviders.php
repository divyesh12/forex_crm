 ...
 * All Rights Reserved.
 * ********************************************************************************** */
include_once dirname(__FILE__) . '/ServiceProvidersBase.php';
include_once dirname(__FILE__) . '/ext/ServiceProvidersProvider.php';
include_once dirname(__FILE__) . '/ext/IServiceProvidersProvider.php';
include_once dirname(__FILE__) . '/ext/ITradingPlatform.php'; // Add by Divyesh 15-11-2019
include_once dirname(__FILE__) . '/ext/IPaymentGetways.php'; // Add by Divyesh 15-11-2019

include_once 'include/Zend/Json.php';

class ServiceProviders extends ServiceProvidersBase {
    ...
    private function processFireSendSMSResponse($responses) {
        ...
        foreach ($responses as $response) {
            ...
            $adb->pquery("INSERT INTO vtiger_serviceproviders_status(serviceprovidersid,tonumber,status,statusmessage,smsmessageid,needlookup) VALUES(?,?,?,?,?,?)", array($this->id, $response['to'], $responseStatus, $responseStatusMessage, $responseID, $needlookup)
            );
        }
    }
    ...
    static function smsquery($record) {
        global $adb;
        $results = array();
        $result = $adb->pquery("SELECT * FROM vtiger_serviceproviders_status WHERE serviceprovidersid = ?", array($record));
        if ($result && $adb->num_rows($result)) {
            $provider = ServiceProvidersManager::getActiveProviderInstance();
            ...
            while ($resultrow = $adb->fetch_array($result)) {
                if ($resultrow['needlookup'] == 1) {
                    ...
                    $adb->pquery("UPDATE vtiger_serviceproviders_status SET status=?, statusmessage=?, needlookup=? WHERE smsmessageid = ?", array($responseStatus, $responseStatusMessage, $needlookup, $messageid));
    ...
    static function getSMSStatusInfo($record) {
        global $adb;
        $results = array();
        $qresult = $adb->pquery("SELECT * FROM vtiger_serviceproviders_status WHERE serviceprovidersid=?", array($record));
...
class ServiceProvidersManager {
     ...
     * @ Add By:- Divyesh
     * @ Date:- 15-11-2019
     * @ Comment:- return object base of provider wise
     */

    static function getActiveProviderInstance() {
        global $adb;
        $result = $adb->pquery("SELECT * FROM vtiger_serviceproviders_servers WHERE isactive = 1 ORDER BY sequence_number ASC", array());
        if ($result && $adb->num_rows($result)) {
            $providers = array();
            while ($resultrow = $adb->fetch_array($result)) {
                $provider = ServiceProvidersProvider::getInstance($resultrow['providertype']);
                $parameters = array();
                if (!empty($resultrow['parameters']) && $provider) {
                    $parameters = Zend_Json::decode(decode_html($resultrow['parameters']));
                    $parameters['title'] = $resultrow['title'];
                    $parameters['providertype'] = $resultrow['providertype'];
    ...
    static function getActiveInstanceByProvider($provider) {
        global $adb;
        $result = $adb->pquery("SELECT * FROM vtiger_serviceproviders_servers WHERE isactive = 1 AND title =? LIMIT 1", array($provider));
        if ($result && $adb->num_rows($result)) {
    ...
    static function getInstanceByProvider($provider) {
        global $adb;
        $result = $adb->pquery("SELECT * FROM vtiger_serviceproviders_servers WHERE  title =? LIMIT 1", array($provider));
        if ($result && $adb->num_rows($result)) {
            $resultrow = $adb->fetch_array($result);
            $provider = ServiceProvidersProvider::getInstance($resultrow['providertype']);
            $parameters = array();
    ...
    static function listConfiguredServer($id) {
        global $adb;
        $result = $adb->pquery("SELECT * FROM vtiger_serviceproviders_servers WHERE id=?", array($id));
        if ($result) {
    ...
    static function listConfiguredServers() {
        global $adb;
        $result = $adb->pquery("SELECT * FROM vtiger_serviceproviders_servers", array());
        $servers = array();
        if ($result) {
    ...
    static function updateConfiguredServer($id, $frmvalues) {
        ...
        if (empty($id)) {
            $adb->pquery("INSERT INTO vtiger_serviceproviders_servers (providertype,username,password,isactive,parameters) VALUES(?,?,?,?,?)", array($providertype, $username, $password, $isactive, $parameters));
        } else {
            $adb->pquery("UPDATE vtiger_serviceproviders_servers SET username=?, password=?, isactive=?, providertype=?, parameters=? WHERE id=?", array($username, $password, $isactive, $providertype, $parameters, $id));
        }
    }

    static function deleteConfiguredServer($id) {
        global $adb;
        $adb->pquery("DELETE FROM vtiger_serviceproviders_servers WHERE id=?", array($id));
    }
    ...
    static function getActiveProviderList($providerType = '') {
        global $adb;
        $result = $adb->pquery("SELECT title FROM vtiger_serviceproviders_servers WHERE isactive = 1 ORDER BY sequence_number ASC", array());
        if ($result && $adb->num_rows($result))
        {
            $providers = array();
            while ($resultrow = $adb->fetch_array($result))
            {
                $provider = ServiceProvidersManager::getInstanceByProvider($resultrow['title']);
                if($providerType === '1')
                {
                    if($provider::PROVIDER_TYPE == 1)
                    {
                        $providerName = $resultrow['title'];
    ...
    static function getActiveMTProviderList() {
        global $adb;
        $result = $adb->pquery("SELECT title FROM vtiger_serviceproviders_servers WHERE isactive = 1 ORDER BY sequence_number ASC", array());
        if ($result && $adb->num_rows($result))
        {

...
class Users_Login_View extends Vtiger_View_Controller {
	...
	function process (Vtiger_Request $request) {
		...
		if ($error) {
			switch ($error) {
				...
				case 'statusError'	:	$message = 'Outgoing mail server was not configured';	break;
                                case 'loginOtpError'	:	$message = $request->get('error_msg');	break;
			}
		} else if ($mailStatus) {
		...
		$loginBackgroundImage = 'test/logo/' . $companyDetails['bannername'];
                        
		$viewer->assign('ERROR', $error);
		$viewer->assign('MESSAGE', vtranslate($message, 'Users'));
		$viewer->assign('MAIL_STATUS', $mailStatus);
		$viewer->assign('SITE_LOGO', $siteLogo);
		$viewer->assign('BACKGROUND_IMAGE', $loginBackgroundImage);

...
Class Reports_Edit_View extends Vtiger_Edit_View {
	...
	public function checkPermission(Vtiger_Request $request) {
		...
		global $customReportids;
		parent::checkPermission($request);
		$recordModel = Reports_Record_Model::getInstanceById($request->get('record'));
		if(!$recordModel->isEditableBySharing()) {
			throw new AppException(vtranslate('LBL_PERMISSION_DENIED'));
		}else if(in_array($request->get('record'), $customReportids)){
			$messege = vtranslate('LBL_PERMISSION_DENIED').' For Custom Report';
			throw new AppException($messege);
		}
		//add by divyesh end
		return true;
	}

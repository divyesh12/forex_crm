...
class Documents_FilePreview_View extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
            global $log,$BUCKET_URL,$s3Enable;
		$moduleName = $request->getModule();
		$recordId = $request->get('record');

		$basicFileTypes = array('txt','csv','ics');
		$imageFileTypes = array('image/gif','image/png','image/jpeg','image/jpg');
		...
		if (!empty ($fileDetails)) {
			...
			if ($recordModel->get('filelocationtype') == 'I') {
				...
				$fileSize = $fileSize + ($fileSize % 1024);
                                if($s3Enable)
                                {
                                    /*AWS S3 code to get file*/
                                    $fileContent = readBucketFile($filePath.$savedFile);
                                    /*AWS S3 code to get file*/ 
                                }
                                else
                                {
		...
		if ($recordModel->get('filestatus') && $recordModel->get('filename') && $recordModel->get('filelocationtype') === 'I') {
                    if($s3Enable)
                    {
                        $downloadUrl =  $BUCKET_URL.$filePath.$savedFile;
                    }
                    else
                    {
		...
		$viewer->assign('s3enable',$s3Enable);
		global $site_URL;

...
class Documents extends CRMEntity {
	...
	function save_module($module){
		...
		//Auto KYC Approved Base on Doc Approved Limit
		$docLimitStatus = configvar('is_doc_limit_enable');
		if($docLimitStatus)
		{
			$docuApprovedLimitForIdProof = configvar('doc_approved_limit_for_id_proof');
			$docuApprovedLimitForResidenceProof = configvar('doc_approved_limit_for_residence_proof');
			$contactId = $this->column_fields['contactid'];
			$record_status = $this->column_fields['record_status'];
			if($contactId && ($docuApprovedLimitForIdProof >= 1 || $docuApprovedLimitForResidenceProof >= 1) && $record_status == 'Approved'){

				$query = "SELECT COUNT(vtiger_notes.notesid) AS no_of_id_proof_approved FROM `vtiger_notes` INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_notes.notesid WHERE vtiger_crmentity.deleted = 0 AND `vtiger_notes`.`record_status` = ? AND vtiger_notes.contactid = ? AND vtiger_notes.sub_document_type = ?";
				$result = $adb->pquery($query, array('Approved',$contactId,'ID Proof'));
				$row = $adb->fetchByAssoc($result);
				$no_of_id_proof_approved = $row['no_of_id_proof_approved'];

				$query2 = "SELECT COUNT(vtiger_notes.notesid) AS no_of_residence_proof_approved FROM `vtiger_notes` INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_notes.notesid WHERE vtiger_crmentity.deleted = 0 AND `vtiger_notes`.`record_status` = ? AND vtiger_notes.contactid = ? AND vtiger_notes.sub_document_type = ?";
				$result2 = $adb->pquery($query2, array('Approved',$contactId,'Residence Proof'));
				$row2 = $adb->fetchByAssoc($result2);
				$no_of_residence_proof_approved = $row2['no_of_residence_proof_approved'];

				if(($no_of_id_proof_approved >= $docuApprovedLimitForIdProof && $no_of_residence_proof_approved >= $docuApprovedLimitForResidenceProof)){
					$adb->pquery("UPDATE `vtiger_contactdetails` SET `is_document_verified`=? WHERE contactid = ?", array(1,$contactId));
				}
			}
		}
	}

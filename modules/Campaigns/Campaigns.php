...
class Campaigns extends CRMEntity {
	...
	*/
	 ...
	 * returns related activities record in array format
	 */
	function get_campaign_activity($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		global $log, $singlepane_view,$currentModule,$current_user;
		$log->debug("Entering get_activities(".$id.") method ...");
		$this_module = $currentModule;
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
							'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
		$query = "SELECT vtiger_campaignactivity.*,
                        vtiger_crmentity.crmid, vtiger_crmentity.smownerid,
			vtiger_crmentity.modifiedtime,
			CASE when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name
			...
			LEFT JOIN vtiger_groups
				ON vtiger_groups.groupid = vtiger_crmentity.smownerid
			WHERE vtiger_campaignactivity.campaign_id=".$id."
			AND vtiger_crmentity.deleted = 0";
...
//           $relatedIds = implode(', ', $relatedIds);
           ...
           if(isset($_REQUEST['campaign_activity_id']) && $_REQUEST['campaign_activity_id'] == '')
           {
               unset($_SESSION['campaign_activity_id']);
           }
           if(isset($_REQUEST['campaign_activity_id']) && !empty($_REQUEST['campaign_activity_id']))
           {
               $where = " AND vtiger_campaignactivity.campaignactivityid = " . $_REQUEST['campaign_activity_id'];
                $_SESSION['campaign_activity_id'] = $_REQUEST['campaign_activity_id'];
           }
            else if(isset($_SESSION['campaign_activity_id']) && !empty($_SESSION['campaign_activity_id']))
            {
                $where = " AND vtiger_campaignactivity.campaignactivityid = " . $_SESSION['campaign_activity_id'];
            }
           $userNameSql = getSqlForNameInDisplayFormat(array('first_name' =>
               'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
           $query = "select case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name," .
                   " vtiger_activity.activityid, vtiger_activity.subject, vtiger_activity.activitytype, vtiger_crmentity.modifiedtime," .
                   " vtiger_crmentity.crmid, vtiger_crmentity.smownerid, vtiger_activity.date_start, vtiger_activity.time_start, vtiger_seactivityrel.crmid as parent_id, vtiger_email_track.access_count,vtiger_email_track.click_count " .
                   " FROM vtiger_activity".
                   ...
                   " WHERE vtiger_campaignactivity.campaign_id = " . $id . " AND vtiger_activity.activitytype = 'Emails' AND vtiger_crmentity.deleted = 0" . $where;

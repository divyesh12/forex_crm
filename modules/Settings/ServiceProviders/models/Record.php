...
class Settings_ServiceProviders_Record_Model extends Settings_Vtiger_Record_Model {
    ...
    public function getRecordLinks() {
        ...
        $recordLinks = array(
            array(
                ...
                'linklabel' => 'LBL_EDIT',
                'linkurl' => "javascript:Settings_ServiceProviders_List_Js.triggerEdit(event, '" . $this->getEditViewUrl() . "');",
                'linkicon' => 'icon-pencil'
            ),
            array(
                'linktype' => 'LISTVIEWRECORD',
                'linklabel' => 'LBL_DELETE',
                'linkurl' => "javascript:Settings_ServiceProviders_List_Js.triggerDelete(event, '" . $this->getDeleteUrl() . "');",
                'linkicon' => 'icon-trash'
            )
        );
     ...
     * @return <Array> field models list <Settings_ServiceProviders_Field_Model>
     */
    ...
    public function save() {
        ...
        $params = array($this->get('providertype'), $this->get('title'), $this->get('isactive'), $this->get('username'), $this->get('password'), $this->get('parameters'), $this->get('sequence_number'));
        $id = $this->getId();
        if ($id) {
            $query = 'UPDATE vtiger_serviceproviders_servers SET providertype = ?,title=?, isactive = ?, username = ?, password = ?, parameters = ?, sequence_number = ? WHERE id = ?';
            array_push($params, $id);
        } else {
            $query = 'INSERT INTO vtiger_serviceproviders_servers(providertype,title, isactive, username, password, parameters, sequence_number) VALUES(?, ?, ?, ?, ?, ?, ?)';
        }
    ...
    static public function getInstanceById($recordId, $qualifiedModuleName) {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT * FROM vtiger_serviceproviders_servers WHERE id = ?', array($recordId));
     ...
     * @param <String> $qualifiedModuleName
     * @return <Settings_ServiceProviders_Record_Model>
     */

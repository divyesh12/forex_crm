...
class Mobile_WS_UserInfo extends Mobile_WS_Controller {
	...
	function process(Mobile_API_Request $request) {
		...
		$query = "SELECT * FROM `vtiger_role` WHERE roleid = '$current_user->roleid' ";
		$result1 = $adb->pquery($query);
		$row = $adb->fetchByAssoc($result1);

		$isAdmin = 'No'; 
		if($current_user->is_admin == 'on' || $current_user->is_admin){
			$isAdmin = 'Yes';
		}
		...
		$userinfo = array(
			...
			'lead_view'=> $current_user->lead_view,

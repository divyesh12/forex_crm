...
class LiveAccount_Save_Action extends Vtiger_Save_Action {
    ...
    public function process(Vtiger_Request $request) {
        ...
        if ($record_status == 'Approved') {
            if (!$account_no) {
                $max_accountNo = LiveAccount_Record_Model::getMetaTradeUpcommingSeqNo($module, $metatrader_type);
...
//  $moduleName = $request->getModule();
                    ...
                    $create_user_result = $provider->createAccount($city, $state, $countryname, $address1, $mailingzip, $mobile, $commnet, $max_accountNo, $password, $investor_password, $phonepassword, str_replace(":", "\\", $account_type), $leverage, $contact_name, $email);
...
//  $moduleName = $request->getModule();
                ...
                if ($recordId && isset($recordId)) {
                    ...
                    if (($Live_modelData['live_label_account_type'] != $label_account_type && $Live_modelData['live_account_type'] != $account_type) || ($Live_modelData['live_account_type'] != $account_type)) {
...
//  $moduleName = $request->getModule();
                        ...
                        $viewer = new Vtiger_Viewer();
                        $viewer->assign('REQUEST_DATA', $requestData);
                        $viewer->assign('REQUEST_URL', $moduleModel->getCreateRecordUrl() . '&error=LEVERAGE_UPDATE_ISSUE&record=' . $request->get('record'));
                        $viewer->view('RedirectToEditView.tpl', 'Vtiger');
                    } else {
                        $requestData = $request->getAll();
//  $moduleName = $request->getModule();
                ...
                if ($is_account_type_changed) {
                    ...
                    if (empty($account_mapping_data)) {
                        ...
                        if ($change_account_type_code == 200 && $change_account_type_messege == 'Ok' && $account_no) {
                            ...
                            $viewer = new Vtiger_Viewer();
                            $viewer->assign('REQUEST_DATA', $requestData);
                            $viewer->assign('REQUEST_URL', $moduleModel->getCreateRecordUrl() . '&error=ACCOUNT_TYPE_UPDATE_ISSUE&record=' . $request->get('record'));
                            $viewer->view('RedirectToEditView.tpl', 'Vtiger');
                        } else {
                            $requestData = $request->getAll();
                            //  $moduleName = $request->getModule();

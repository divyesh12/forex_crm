...
abstract class CustomerPortal_API_Abstract {
    ...
    function getUserDataById($id) {
        ...
        $sql = "SELECT user_name, user_password FROM vtiger_portalinfo "
                . "INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_portalinfo.id "
                . "INNER JOIN vtiger_customerdetails ON "
                . "vtiger_customerdetails.customerid=vtiger_portalinfo.id "
                . "WHERE vtiger_crmentity.deleted=0 AND vtiger_portalinfo.id=? AND isactive=1 AND "
                . "vtiger_customerdetails.portal=1";

        $result = $adb->pquery($sql, array($id));
        $num_rows = $adb->num_rows($result);
        $isAuthenticated = false;
        if ($num_rows >= 0) {
            return $user_name = $adb->query_result($result, $i, 'user_name');
        } else {
            return null;
        }
    }

    function authenticatePortalUser($username, $password, $pass_check) {
        global $adb;
        $current_date = date("Y-m-d");
        $sql = "SELECT id, portal_language, user_name, user_password,last_login_time, isactive, support_start_date, support_end_date, cryptmode FROM vtiger_portalinfo
        ...
        if ($num_rows >= 0) {
            ...
            if ($customerId) {
                ...
                if ($isActive && ($support_end_date >= $current_date || $support_end_date == null)) {
                    ...
                    $current_customer->portal_language = $adb->query_result($result, $i, 'portal_language');
                ...
                throw new Exception(vtranslate('CAB_MSG_ACCESS_TO_PORTAL_WAS_DISABLED_ON', $this->translate_module, $this->getActiveCustomer()->portal_language) . $support_end_date, 1413);
            } else if ($isActive === 0) {            
                throw new Exception('CAB_MSG_PORTAL_ACCESS_HAS_BEEN_ENABLED', 1414);
            } 
        }

	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			if (!CustomerPortal_Utils::isModuleActive($module)) {
				throw new Exception(vtranslate('CAB_MSG_HISTORY_NOT_AVAILABLE_FOR_THIS_MODULE', $this->translate_module, $portal_language), 1412);
				exit;
			}

			if (!ModTracker::isTrackingEnabledForModule($module)) {
				throw new Exception(vtranslate('CAB_MSG_MODULE_NOT_TRACKED_FOR_CHANGES', $this->translate_module, $portal_language), 1412);
				exit;
			}
			...
			if (empty($relatedIds)) {
				throw new Exception(vtranslate('CAB_MSG_NO_RECORDS_FOUND', $this->translate_module, $portal_language), 1412);
				exit;
			}
			$recordIds = array();
			...
			$response->setError(1404, vtranslate('CAB_MSG_NO_PERMISSION_TO_PERFORM_THIS_OPERATION', $this->translate_module, $portal_language));			
		}
		return $response;
	}

		...
		if ($current_user) {
			...
			if (!empty($parentId)) {
				...
				if (!$this->isFaqPublished($recordId)) {
					throw new Exception(vtranslate('CAB_MSG_THIS_FAQ_IS_NOT_PUBLISHED', 'Faq', $portal_language), 1412);					
			...
			if ($module == 'ModComments' && !empty($recordId)) {
				...
				if (!CustomerPortal_Utils::isModuleActive($relatedModule)) {
					throw new Exception(vtranslate('CAB_MSG_COMMENTS_NOT_ACCESSBILE_FOR_THIS_RECORD', $this->translate_module, $portal_language), 1412);					
					exit;
				}

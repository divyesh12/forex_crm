...
//						3 => 'LBL_PREFER_VTIGER_RECORD',
	...
	public function getModuleIcon($isRelatedTolist = false) {
		$moduleName = $this->getName();
        ...
        $moduleImageName = $moduleName;
        $imageClass = 'module-sidebar-icon';
        if($isRelatedTolist)
        {
            $moduleImageName = $moduleName . '_list';
            $imageClass = 'module-related-icon';
        }
		$imageFilePath = 'layouts/'.Vtiger_Viewer::getLayoutName()."/modules/$moduleName/$moduleImageName.svg";
		if (file_exists($imageFilePath)) {
			$moduleIcon = "<img class='$imageClass' src='$imageFilePath' title='$title' style='vertical-align: top;'/>";
		}

		return $moduleIcon;
	}
    
    public function getModuleIconUseFaIcon()
    {
        $moduleName = $this->getName();
        $lowerModuleName = strtolower($moduleName);
        $title = vtranslate($moduleName, $moduleName);
        $custom_icon_name = $lowerModuleName . '_icon';
        $custom_icon = VtigerConfig::get($custom_icon_name);
        $moduleIcon = "<i class='vicon-$lowerModuleName' title='$title' test1= '$lowerModuleName'></i>";
        if ($this->source == 'custom' || $custom_icon != '')
        {
            ...
            {
                $moduleIcon = "<i class='$custom_icon' title='$title' test='$lowerModuleName' test1= '$custom_icon'></i>";
            }
        }
	...
	public static function getModuleIconPath($moduleName, $isDiffIcon = false) {
		$moduleModel = Vtiger_Module_Model::getInstance($moduleName);
		return $moduleModel->getModuleIcon($isDiffIcon);
	}
}
